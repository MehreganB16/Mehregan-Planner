...
const priorityConfig: Record<Priority, { label: string; color: string; icon: React.ElementType }> = {
  ...
  low: { label: 'Low', color: 'bg-secondary text-secondary-foreground hover:bg-secondary/90', icon: ChevronDown },
};
  ...
  return (
    <Card className={cn(
      ...
      <CardContent className="p-4 flex items-start gap-4">
        ...
        <Button variant="ghost" size="icon" className="h-8 w-8 flex-shrink-0" aria-label="Delete task" onClick={() => onDelete(task.id)}>
